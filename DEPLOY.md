...
Полная проверка функций (обе — без зависимостей, только стандартная библиотека):

```bash
backend/.venv/bin/python scripts/smoke_deploy.py https://<домен>   # HTTP, SPA, документы, WS, live-перевод
backend/.venv/bin/python scripts/smoke_llm.py    https://<домен>   # ИИ-разметка документа + объяснение фрагмента
```

Вторая проверка обращается к LLM: загружает отрывок из «Лунь юй», запускает
ИИ-разметку, ожидает её завершения и просит объяснить фрагмент `学而时习之`.
Признак успеха — `engine=llm`, `degraded=False`.
